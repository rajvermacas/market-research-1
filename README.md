...
| Loop-26 screens (`strat_l26a_corrspike` / `xuniconfirm` on the Nifty 500 book; `strat_l26b_upmonths` / `breakmag` on the L25 champion) | 2015→2026, monthly, `--exec realistic`, forward blind; first loop run under the MANDATORY no-repeat rule | Nifty 500 drawdown controls: correlation-spike cut knife-edge (≤0.4pp DD), shock exit 33.3/−10.5 on ~4 events, cross-universe breadth deepens DD. Full board: trend consistency dead both signs; **breakout magnitude** (+w) a broad train plateau 31.5–34.6 / −16.1…−17.6, gated w .2 / lb 9: train 33.4/−16.1 → fwd **18.3/−14.3** (calmar 1.27) vs champion 21.6/−20.5 (1.05) | **No promotion — the L25 champion stands.** Breakout magnitude is the best forward-drawdown line ever found (−14.3%) but gives back 3.3pp of forward CAGR, so it fails the CAGR-first trade-off rule (and the deflated margin over 300 trials) — recorded as a risk-lean alternative. 11 reveals used. |
| Loop-27 screens (regime: `strat_l27a_dispersion` / `rvpct` / `sizeflight` / `froth`; rank: `strat_l27b_spread` / `idiomom`, `strat_l27b2_amihud` / `roll` / `zeroret` + compositions) on the L25 champion | 2015→2026, monthly, `--exec realistic`, 50 bps round trip, forward blind until the gate | Dispersion, board realized-vol and flight-to-quality exposure cuts dead (DD never moves). Illiquidity rank premium (wide CS spread / high Amihud) a broad train plateau ~35/−16; gated CS spread w .2 lb 9: train 35.19/−15.84 → fwd **18.35/−17.24** vs champion 21.58/−20.47. Froth exposure cut (illiquid tail > liquid core by 6pp over 2 months → exposure ×0.6): train 30.21/−14.09 → fwd **23.44/−14.44**, all 9 gate checks pass | **No promotion — the L25 champion stands.** Spread fails the CAGR-first rule (−3.2pp fwd). Froth was crowned and then **revoked**: its sign was chosen after a diagnostic read the Jan-2022 (forward-window) spike, and those forward firings carry its forward DD gain, so the pass is not out-of-sample. Kept on record. Also: lock-down panic cut and liquid-core slump cut dead; liquid-core rebound re-arm reads 39.2/−17.2 train but rests on the 2020 V (7 firings) — an ungated lead. 15 files, 150 trials, 13 reveals used. |
| Loop-28 screens (exposure: `strat_l28a_ipofroth` / `tvsurge` / `thrust` / `nhconfirm`, `strat_l28c_upvol`, `strat_l28o_upvolnz`; book construction: `strat_l28b_corrdiv` / `volcap` / `betacap`, `strat_l28d_lockdemote` / `runupdemote`; policy min_hold / max_weight / rank) on the L25 champion | 2015→2026, monthly, `--exec realistic`, 50 bps round trip, forward blind until the gate | IPO-wave, turnover-surge, new-high re-arm, lock-demote, vol/beta caps dead. Correlation-diversified selection + rank weights: train 33.16/−17.63 → fwd **18.09/−18.50** vs champion 21.58/−20.47 (bench 14.53/−27.88). Up-value-share re-arm from a partial tier (42d, UVS > .52 → full): train 38.46/−17.76 (plateau .48–.55) → fwd **26.79/−26.49** | **No promotion — the L25 champion stands.** corrdiv loses 3.5pp forward CAGR (the fifth selection-side gain that dies forward). upvolnz adds 5.2pp forward CAGR but its DD is 6.0pp deeper (2.6pp allowed): re-arming buys return with drawdown on the forward window. Both also fail the best-of-N margin check. Breadth thrust and run-up demote are ungated train leads. 11 files, 146 trial rows (5 identity checks) + 2 gates, 15 reveals used. |
| Loop-29 screens + random null controls (exposure: `strat_l29a_breadthspread` / `regimeage` / `signalhit` / `capture` / `agerearmup`, `strat_l29f_fallcut`; Nifty 500 DD: `strat_l29b_bookdd` / `bookvol` / `midbeta`; changed-base legacy tilts: `strat_l29c_insideday` / `listage` / `actionclass` / `spreadz`; book: `strat_l29e_coresat`, `strat_l29g_liqcore`; rank: `strat_l29j_liqclimb`; controls: `strat_l29c_randtilt`, `strat_l29d_randrearm`, `strat_l29f_randcut`, `strat_l29h_randres`, `strat_l29i_n500null`) on the L25 champion | 2015→2026, monthly, `--exec realistic`, 50 bps round trip, forward blind until the gate | Random rank tilts: train 30.46 ± 1.29 (9/16 beat the champion's 30.09). Random partial-tier re-arms: 34.18 ± 2.14. Liquidity-rank-climb tilt (favour names falling in traded-value rank, −0.2 / 6m): train 36.32/−15.21, above all 24 random-tilt draws → fwd **17.28/−19.54** vs champion 21.58/−20.47 (bench 14.53/−27.88) | **No promotion — the L25 champion stands.** The one gated line loses 4.3pp forward CAGR: the sixth selection-side train gain that dies forward, and the first that had cleared a random null. Re-arm and rank-tilt train gains are mostly exposure/noise (see the null bands). A today's-Nifty-500 slot reservation (36.02/−16.20) is look-ahead: its point-in-time liquidity version loses. 21 files, 238 trial rows + 1 gate, 16 reveals used. |
| **Loop-25: NEW CHAMPION `strat_l23a_liqconfirm` + 25 names inverse-vol** (liquid-cohort breadth regime with shifted tier thresholds `lq_min 5e6 / lq_days 40 / lq_shift .05 / lq_shift_lo .075 / strict` on the fresh-print base; policy `top 25`, `weighting invvol`) | 2015→2026, monthly, full NSE board, `--exec realistic` (next-open fill, lock block, ₹50 lakh floor, 50 bps), forward revealed once by the gate | Train **+30.1%** / DD **−17.2%** (calmar 1.75) vs old champion +24.1%/−19.0% and bench +19.8%/−62.7%; forward **+21.6%** / DD **−20.5%** (calmar 1.05) vs old champion +20.6%/−21.7% and bench +14.5%/−27.9%. Nifty 500 transfer +28.4%/−12.1% vs bench +21.1% | **Promoted — all 9 gate checks pass and it dominates on both windows.** Neighbours 23/26 hold ≥80% of robust; 2× cost 27.3%; noise p10 1.82; deflated margin +1.06 vs required 0.60 over 177 trials; footprint 26/94 months. The forward edge over the old champion is modest (+0.95pp CAGR, 1.2pp shallower DD). Same loop: the Nifty 500 as a primary universe (84 trials) found a slower regime (`regime_ma 24`, `b_mid .58`, `floor_lb 24`) at train +32.8%/−11.4%, gated to fwd **22.8/−25.7** — +1.2pp CAGR for 5.2pp more DD, fails the trade-off rule; gap-drift on the new champion lowers train CAGR; 20–24-name books are a train plateau (+31.6–33.4 / −16.4…−17.1), but the gated top-22 cell read fwd **20.6/−18.7** (−0.9pp CAGR vs the new champion) — concentration loses forward again. 10 reveals used. |
| **Loop-24: risk policy moves into training** (`strategy_lab.py` policy keys: `top`, `weighting` equal/rank/invvol, `max_weight`, `min_hold`, `max_hold`, `sl_pct` / `ts_pct` stops simulated on the daily path, `stop_cool`) | 2015→2026, monthly, full NSE board, `--exec realistic`, ruler = median train-fold calmar, forward blind | On the champion: top 15 equal 27.0/−18.8; **top 15 inverse-vol 30.0/−17.9** (plateau 12–20 names); rank weights 25.3/−21.3; every stop worse (sl 15–35%: 19.4–25.5 / −20.4…−21.7; trailing 10–35%: 5.4–24.8); min/max hold and weight caps no gain. With the liqconfirm regime: top 15 inverse-vol 34.6/−17.9 (robust 3.23); top 25 inverse-vol 30.1/−17.2 (all neighbours 26.8–33.4 / −16.8…−18.4). Gated: top-15 inverse-vol fwd **15.8/−20.5**; liqconfirm + top-15 inverse-vol fwd **16.2/−17.7** (calmar 0.92) vs champion 20.6/−21.7 | **No promotion — the champion stands.** The harness now owns only the market's rules (fills, locks, liquidity, costs), the ruler and the gate; buying, sizing, holding and stops are searched per trial. Unit-tested stop fills (level, gap-through, locked day); the plain book is unchanged bit-for-bit and the weighted path with no-op keys matches to 1e-16. Finding: the train ruler prefers concentrated 12–15-name books, and both such candidates lost ~4.5pp of forward CAGR; stops are dead on this base. Lead for L25: liqconfirm + top 25 inverse-vol (ungated). 7 reveals used. |
| **Loop-23: first loop on the blind/robust harness + promotion gate** (8 mechanisms: `strat_l23a_liqconfirm` / `newlows` / `liqrank`, `strat_l23b_ddquality` / `gapdrift` / `driftdq` / `trmom`, `strat_l23o_liqdq`) | 2015→2026, monthly, full NSE board, top 25, `--exec realistic`, ruler = median train-fold calmar, forward blind | Champion `strat_floorhighfresh` (first realistic champion, passed all gate checks): train +24.1%/−19.0% (calmar 1.27), fwd +20.6%/−21.7% (calmar 0.95) vs bench +19.8% / +14.5% (−27.9%). Gated candidates (one logged reveal each): ddquality −0.15/4 train 27.3/−16.4 → fwd **16.9/−21.5**; liqconfirm train 28.5/−16.3 → fwd **20.4/−19.2** (calmar 1.06); liqdq composition train 31.9/−16.6 (calmar 1.92) → fwd **17.1/−19.4** | **No promotion — the champion stands.** Every gated candidate beat the champion on train CAGR and DD by far more than its noise margin and passed neighbours, 2× cost, noise, footprint (except liqconfirm, 9/94 months) and Nifty 500 transfer — then fell short of the champion's forward CAGR, so the dominance check refused all three. The drawdown-path tilt (ddquality, favouring V-recovering names) is the recurring culprit: a clean train plateau that costs 3.5pp of forward CAGR alone and in composition. liqconfirm is the nearest miss (better fwd DD, −0.2pp fwd CAGR). A 9th mechanism in the remaining clock, `strat_l23o_liqramp` (continuous exposure ramp on liquid breadth), kept on train (25.3/−16.7, robust 1.645) and gated to fwd **19.2/−19.1** — the regime channel consistently buys forward DD with forward CAGR, so it cannot pass dominance alone. 126 trials, 5 reveals total. Second gate fix: footprint now counts exposure changes, not only book changes (liqramp 45/97, liqconfirm 25/94 months). Harness fix from this validation: the gate's best-of-N margin now counts trials across every realistic ledger (112), not only the main one. |
| Loop-22 execution-aware screens on the realistic base (`strat_l22a_advtilt` / `lockpen` / `gappen`, `strat_l22b_liqbreadth` / `liqrs`; base `strat_floorhighfresh` top 25, the first `best_real.json` entry) | 2015→2026, monthly, full NSE board, top 25, `--exec realistic` (next-open, lock block, ₹50 lakh floor, 50 bps) | Base train +24.1%/−19.0% (calmar 1.27), fwd +20.6%/−21.7% vs bench +19.8% / +14.5% (fwd DD −27.9%). Traded-value tilt 21.1–22.5, fwd 11.8–16.6; lock-share penalty 20.7–22.1, fwd 10.6–12.6; overnight-gap penalty 23.2–23.9, fwd 9.6–14.1; liquid-only RS 20.6–20.8, fwd 9.4–15.9; liquid-only breadth regime 26.0–28.3 at DD −24.4…−25.1 (calmar 1.07–1.13), fwd 22.3–22.5/−19.5 | **No promotion — the realistic base stands.** All five off-switches reproduce the base exactly. Ranking on fillability swaps away momentum signal: every tilt cuts the blocked-entry counts but loses return on both windows, so the harness's own refusal already prices execution. Liquid-only breadth is the one lead (higher CAGR both windows, better fwd DD) but pays ~6pp train DD through higher exposure — follow up with higher breadth thresholds. 10-minute loop: 2 cells per file, no neighbourhood screens, no audit, no Nifty 500 validation. Ledgers: `.cache/strategy_lab/best_real.json`, `l22_dA_*`, `l22_dB_*` |
| **Execution re-rank of the strategy_lab lineage** (`strategy_lab.py --exec realistic`: fill at the next session's open, refuse entries whose execution bar is circuit-locked up (high = low above prior close) or zero-volume, trap exits locked down, INR 50 lakh 20-session median traded value for new entries, 50 bps round trip + half on exposure changes) | 2015→2026, monthly, full NSE board, each line at its own top N; bench +19.8% train / +14.5% fwd (fwd bench DD −27.9%) | Legacy → realistic, train CAGR/DD · fwd CAGR/DD: **L21 spreadz** 104.5/−15.0 · 55.0/−11.0 → **39.2/−18.6 · −5.2/−41.2**; L20 spread 103.0/−15.5 · 54.1/−10.4 → 38.9/−18.6 · −5.2/−41.2; L19 balanced 98.7/−15.5 · 53.2/−10.4 → 38.7/−18.6 · −5.2/−41.2; L15 insideday 96.9/−17.0 · 49.0/−12.3 → 38.1/−18.6 · −4.8/−41.2; L13 concwobble 88.1/−17.3 · 49.3/−14.9 → 44.8/−16.1 · 3.2/−24.6; L12 tiershapeconc 85.2/−17.5 · 53.8/−14.4 → 45.3/−19.5 · 10.3/−24.7; symcap 84.9/−19.2 · 43.1/−15.9 → 36.5/−30.9 · 10.4/−31.4; rankpersist 83.9/−20.0 · 49.9/−16.4 → 34.9/−34.6 · 20.2/−20.1; tiershape 77.9/−19.0 · 50.0/−14.4 → 39.2/−34.0 · 20.4/−27.5; sustaincond 67.0/−19.7 · 40.3/−14.4 → 32.9/−19.9 · 12.8/−26.2; top-25 fresh-print family (floorhighfresh/fastgate/sustainprint/tier) 45–48/−17 · 32–33/−14 → 23–24/−19…−21 · **19.5–20.7/−20.5…−21.7**; newhigh top 25 → 23.1/−22.9 · 3.7/−25.5 | **The Loop-21 champion is an execution artifact; no strategy_lab line survives as a champion.** A review of the L21 champion found ~30% of its entries closed at their high and ~16% printed one price all day — upper-circuit locks with no sellers, bought at the same month-end close the signal read. Refusing only those entries took fwd to +20.9/−39.4, far outside a random-block control that refuses the same 11.7% of entries at random (fwd 42–62, DD median −17.5); a one-day fill delay alone costs ~9pp of forward; a INR 1 cr/day floor takes the champion to fwd −4.2/−46.2. Every tilt since L15 (inside-day, age/outrank, spread level, spread z) is **inert** under realistic execution — identical fwd −5.19/−41.19 — because they only re-order names that the lock/liquidity blocks remove. Under realistic execution the legacy ratchet ranks backwards: the older, broader top-25 fresh-print books keep ~+20% fwd at a DD better than the bench, while the L15–L21 chain loses money forward. Nothing here is promoted: the realistic ledger (`.cache/strategy_lab/results_real.tsv` / `best_real.json`) starts empty and the next loop must re-establish a baseline under it. `--exec legacy` reproduces every legacy number above bit-exactly, and a best measured under one harness stamp can no longer rank a trial run under another. |
| **Own-history spread z-score** `strat_l21b_spreadz` (sz_lb 24 / sz_mode `z` / sz_w −0.05 on the L20 spread champion chain) | 2015→2026, monthly, full NSE board, top 15, 25 bps | Train **+104.5%** CAGR at **−15.0%** DD (calmar **6.98**) vs equal-weight +19.8%/−62.7%; forward **+55.0%** at **−11.0%** DD (fwd calmar 5.01); full decade +83.2%/−21.5% (calmar 3.87); worst complete year −15.0%; yearly stdev 170 | **New champion (Loop-21)** — promoted on the main CAGR ratchet: train +1.46pp CAGR and 0.55pp shallower DD vs the L20 line; forward +0.94pp CAGR at a 0.60pp fwd-DD cost (fwd calmar 5.01 vs 5.21) — a **return-lean upgrade, not a risk-clean one**; full-window DD 0.76pp deeper (calmar 3.87 vs 3.96). Signal: the champion's own spread level (CS trailing mean, sp_lb 4) z-scored against the name's OWN trailing 24-month history (mean/std over prior decision rows only, ddof 0, flat histories earn no z); the tested direction favours names whose spread is **abnormally tight for themselves** (`sz_w −0.05`). Robustness: lb plateau is flat (12:103.71, 18:104.41, 24:104.45, 30:104.49, 36:104.49); the weight response is a smooth hump whose whole −0.035..−0.065 neighbourhood beats the champion (103.38 / 104.35 / 104.45 / 103.64 / 103.54), so no knife-edge; ownpct normaliser 104.11/−15.51; positive sign 98.74; book footprint **22/139 decision months** (broad, not a handful). Cost slopes: 50bps → 102.06/−15.65 (fwd 53.12/−11.16), 100bps → 97.34/−17.03 (fwd 49.41/−11.52). Window splits: from 2018-01 train 83.96/−12.37, from 2020-01 261.5/−11.8 — forward identical at 54.997/−10.98 in every split run. Sibling findings from the same axis: the spread **change** (first difference) peaked at lb 12 (103.85/−14.88) but its lb curve oscillates (10:102.65, 14:102.25) — declined as a knife-edge; spread dispersion (both signs) trails on train, but its **Roll-1984** variant (6m, −0.05) posts the loop's best forward (+55.68%/−10.11%, fwd calmar 5.51) at a sub-champion train (102.31) — a forward-only sibling, first Roll use in-tree. Companion L21 findings: illiquidity × dividend-factor composition interferes on train (101.80, below both parents) though forward-additive (+56.96, loop-best fwd); print-coverage (availability) adds +0.71pp train at identical DD but breaks fwd DD 2.70pp at both lags (declined); corporate-action class recency (recent lumpy factor events) reads train 105.55 but gives back 2.48pp of fwd CAGR (declined, artifact); the book-state churn family closes — re-entry lockout destroys monotonically (K1 91.6 → K6 56.1), while new-entry seasoning and exit-memory gains rest on 9 and 7–13 of 139 decision months (handfuls, declined by their own pre-registered falsifiers). Nifty 500 transfer fails as always (+37.3/−23.9 train, fwd +19.8/−31.4). Audit (glm-5.3 max, clean room): promoted row and off-switch reproduced exactly; score-level **bitwise** identity (scores + regime `np.array_equal`; rows 0–27 bitwise equal, z-tilt starts row 28); PIT vintage test at 6 cutoffs (incl. two mid-month) → **3,952,110 pre-cutoff cells, 0 differences**; 362,793 symbol-buckets, 0 boundary violations; footprint independently re-derived 22/139 (47 name-months); plateau claims supported (weight hump, not knife-edge); both higher-train declines (actionclass, spreadchg) verified correct. Ledger: `.cache/strategy_lab/best.json` |
