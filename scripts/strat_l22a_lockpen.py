"""Candidate: up-lock / zero-volume session penalty (strategy_lab contract; Loop-22 designer A, axis =
EXECUTION AWARENESS).
...
the share of sessions that were up-locked (high==low above prior close) or zero-volume, averaged (sign flipped so fewer locks = higher pct)
...
Hypothesis: names that locked or went untraded recently are the ones whose next-open entries get refused or filled at a gap; demoting them keeps the book fillable.
...
the L20 chain). What changed: the lock-share channel itself (no registry row measures circuit-lock incidence), and it is measured for the first time
under --exec realistic (next-open fill, lock/zero-volume refusal, 50 lakh
ADV floor), where fillability is priced by the harness.

Keys consumed: lk_w, lk_lb (w default 0.0 = off-switch identity; NaN stays NaN).
...
NEEDS_DAILY = True
SPACE = {"lk_w": [0.0, 0.2, 0.4], "lk_lb": [2, 3]}
...
def _monthly_feature(daily: pl.DataFrame) -> pl.DataFrame:
    ...
    return d.group_by(["symbol", "mon"]).agg((((pl.col("high") == pl.col("low")) & (pl.col("close") > pl.col("pc"))) | (pl.col("volume") == 0)).cast(pl.Float64).mean().alias("f"))


def score(panels, params):
    s, regime = _base(panels, params)
    w = float(params.get("lk_w", 0.0))
    if w == 0.0:
        return s, regime
    lb = int(params.get("lk_lb", 2))
